...
function App(): React.JSX.Element {
  ...
  type ExtractionMode = 'mistral' | 'local_llm';
  ...
  const handleScanAndExtract = async () => {
    ...
    try {
      ...
      if (extractionMode === 'local_llm') {
        ...
        // Full Mistral pipeline via pluggable provider
        const provider = new MistralProvider({ apiKey: MISTRAL_API_KEY });
        const result = await extractDocument(
          provider,
          results[0].uri,
          { documentType: selectedDocType, language: 'vi' },
          controller.signal,
        );
        const enriched = enrichExtractionResult(result);
        setExtractionResult(enriched);
          ...
          {appMode === 'extract-pick' && !cameraActive && (
            <View style={styles.resultContainer}>
              ...
              <TouchableOpacity
                style={[
                  styles.ocrToggle,
                  extractionMode === 'mistral' && styles.ocrToggleActive,
                ]}
                onPress={() => setExtractionMode('mistral')}
              >
                <Text
                  style={[
                    styles.ocrToggleText,
                    extractionMode === 'mistral' && styles.ocrToggleTextActive,
                  ]}
                >
                  Mistral (online)
                </Text>
                <Text style={styles.ocrToggleHint}>
                  Uses Mistral OCR + Chat API - requires API key & network
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
